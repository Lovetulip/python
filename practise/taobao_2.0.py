...
class myThread(threading.Thread):
    ...
    def __init__(self,name,girlHRUL,outputDir):

        threading.Thread.__init__(self)
        self.girlsHURL = girlHRUL
        self.outputDir = outputDir
        self.name      = name
    def run():

        print("milt task is starting")

        try :
            print("start process" + self.name)
            getImgs(girlHRUL,outputDir)
            print("exit proces" + self.name )
...
#        getImgs(girlHRUL, outputDir + girlNL)
    ...
    for tname in Threadlist:
        ...
        thread = myThread(tname,workQueue,outputDir+girlNL)
        thread.start()
        threads.append(thread)
    for url in girlsPhotoURL:
        workQueue.put(url)
